...
class TripletSamplerView(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        self.dataset = dataset
        self.batch_size = batch_size
        if len(self.batch_size) != 2:
    ...
    def __iter__(self):
        while True:
            ...
            for pid in pid_list:
                indices = []
                while len(indices) == 0:
                    view_id = sync_random_sample_list(
                        self.dataset.views_set, 1)
                    _index = self.dataset.indices_dict.loc[pid, :, view_id].values
