...
#CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=4 --master_addr=127.0.0.1 --master_port=50005 opengait/main.py --cfgs ./configs/gaitgl/gaitgl_CCGR_MINI.yaml --phase test --log_to_file &&
#CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nproc_per_node=4 --master_addr=127.0.0.1 --master_port=50005 opengait/main.py --cfgs ./configs/gaitgl/gaitgl_CCGR_MINI_PAR.yaml --phase test --log_to_file &&
